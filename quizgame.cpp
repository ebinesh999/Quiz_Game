...
int main()
 ...
 if (nq[w]==r) goto start;
  ...
  {
  case 1:
  printf("\n\nHow many runs Suresh Raina scored in ODI?");
  printf("\n\nA.5,615\tB.10,987\n\nC.12,900\tD.9,115\n\n");
  countq++;
  if (toupper(getch())=='A')
   {printf("\n\nCorrect!!!");countr++; break;}
  else
         {printf("\n\nWrong!!! The correct answer is A.5,615");break;}

  case 2:
  printf("\n\n\nSuresh Raina retirement On the same date which palyer retired from international cricket?");
  printf("\n\nA.Dhoni\tB.Kohli\n\nC.Maxwell\tD.Yuvaraj\n\n");
  countq++;
  if (toupper(getch())=='A')
   {printf("\n\nCorrect!!!");countr++; break;}
  else
         printf("\n\nWrong!!! The correct answer is A.Dhoni");
  break;
